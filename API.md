...
| `ALIAS_LIMIT_REACHED` | 400 | 单账号活跃别名已达 Apple 500 上限，网关层自动熔断拦截 |
...
- **`auto_delete=true` 自动闭环回收**：命中并返回验证码后，系统自动在后台异步停用该别名，即刻释放 Apple 500 别名配额，实现无需人工介入的无限循环注册！
- **参数说明**：
...
3. **500 别名硬顶熔断保护**：
   单个 Apple ID 活跃别名官方上限为 500 个。网关层在出号与调度时实时核验，触顶时自动故障转移（Failover）至下一个健康账号，并在达到时返回 `400 ALIAS_LIMIT_REACHED`，严禁触碰 Apple 上游错误风控。
