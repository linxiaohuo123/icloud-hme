...
func TestPR04_A01_CrossTokenVerificationIsolation(t *testing.T) {
	...
	allocB, err := st.ClaimInventoryAlias(context.Background(), "token", tokB.ID, "allocate", "k_b", "h", "tag", "")
...
func TestPR04_A04_TokenRenameAndRecreateNoDataLeak(t *testing.T) {
	...
	_, _ = st.ClaimInventoryAlias(context.Background(), "token", tokOld.ID, "allocate", "k_cs", "h", "tag", "")
...
func TestPR04_A05_TokenRevocationDuringLongPoll(t *testing.T) {
	...
	_, _ = st.ClaimInventoryAlias(context.Background(), "token", tok.ID, "allocate", "k_eph", "h", "tag", "")
