 ...
 * [OUTPUT]: 提供 TestMIG_MID_01 至 TestMIG_MID_04 中间态迁移与架构演进回归单测
...
func TestMIG_MID_02_PR05_To_PR08_InventoryStateQuarantine(t *testing.T) {
	...
	// 2. leased_mid 归属必须为 legacy_unknown，新创建的同名 token 绝不可越权认领其 allocation
	alloc, err := st.GetPrincipalAllocation(ctx, "token", "tok_mid_new", "lease_mid")
