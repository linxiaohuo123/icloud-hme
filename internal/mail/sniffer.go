 ...
 * [POS]: internal/mail 的验证码与激活链接嗅探器，供 server/verify_handler 消费；支持正反向语序与修饰定语容差提取，防守 \bpin\b 与 \bcode\b 词边界
...
var (
	...
	contextCodeRegex = regexp.MustCompile(`(?i)(?:验证码|校验码|动态码|确认码|安全码|动态口令|口令|one-time\s*password|temporary\s*password|verification\s*code|security\s*code|verify\s*code|auth\s*code|passcode|\bcode\b|otp|pin)[^\r\n\d]{0,12}?(?:is|为|是)?[:：\s-]*\b([0-9]{4,8})\b`)

	// contextHyphenCodeRegex 匹配带上下文关键词的连字号验证码 (例: "code: 123-456", "验证码 839-201", "pin is 123 456")
	contextHyphenCodeRegex = regexp.MustCompile(`(?i)(?:验证码|校验码|动态码|确认码|安全码|动态口令|口令|one-time\s*password|temporary\s*password|verification\s*code|security\s*code|verify\s*code|auth\s*code|passcode|\bcode\b|otp|pin)[^\r\n\d]{0,12}?(?:is|为|是)?[:：\s-]*\b([0-9]{3,4}[-\s][0-9]{3,4})\b`)
	...
	reverseContextCodeRegex = regexp.MustCompile(`(?i)\b([0-9]{4,8})\b[^\r\n\d]{0,20}?(?:is(?:\s+(?:your|the|a|an))?|为(?:您(?:的)?)?|是(?:你(?:的)?)?|为本次|作为)?[^\r\n\d]{0,20}?(?:验证码|校验码|动态码|确认码|安全码|动态口令|口令|one-time\s*password|temporary\s*password|verification\s*code|security\s*code|verify\s*code|auth\s*code|passcode|\bcode\b|otp|\bpin\b)`)
